...
# Streamlit Application Specification: Model Explanation & Explainability Control Workbench

## 1. Application Overview

The **Model Explanation & Explainability Control Workbench** is a Streamlit application designed to operationalize model interpretability as a formal enterprise control, rather than merely a visualization exercise. It provides a robust framework for Model Validators, Internal Auditors, and ML Engineers to assess, explain, and document the behavior of machine learning models in a reproducible and defensible manner, specifically addressing the enterprise question: "Can this model's decisions be explained to auditors, regulators, and internal stakeholders in a reproducible and defensible way?"

The application's core purpose is to generate audit-ready explanation artifacts that are tied to specific model versions, reproducible on demand, and suitable for rigorous review.

**High-level Story Flow (Persona: Anya Sharma, Model Validator at PrimeCredit Bank):**

1.  **Welcome:** Anya is introduced to the workbench, understanding her mission to validate PrimeCredit Bank's new Credit Approval Model (CAM v1.2) for transparency, fairness, and compliance.
2.  **Setup & Data Ingestion:** Anya begins by uploading the CAM v1.2 (`sample_credit_model.pkl`) and its associated feature dataset (`sample_credit_data.csv`). The system calculates and displays cryptographic SHA-256 hashes for both artifacts, establishing a foundational element for auditability and traceability.
3.  **Global Explanations:** To understand the model's overall behavior, Anya generates global feature importance using SHAP. She analyzes which factors (e.g., `credit_score`, `income`) generally drive loan approval decisions across the entire dataset, verifying alignment with PrimeCredit's lending policies.
4.  **Local Explanations:** For a deeper dive into specific decisions, Anya selects individual loan applications (e.g., a denied case, an approved case, a borderline case) and generates local SHAP explanations. This allows her to examine the exact contributions of each feature to that particular prediction, providing clear, defensible reasons for individual outcomes.
5.  **Counterfactuals:** Anya then investigates "what if?" scenarios for a denied applicant. Using DiCE, she generates counterfactual examples, identifying the minimal changes to features (e.g., `credit_score`, `income`) that would have resulted in loan approval. This offers actionable insights for customer feedback and model sensitivity analysis.
6.  **Summary & Audit Trail:** Finally, Anya synthesizes her findings into a comprehensive validation summary report, identifying interpretability gaps and providing recommendations for deployment. All generated explanations, configurations, and an evidence manifest (containing SHA-256 hashes of all artifacts) are then bundled into a timestamped ZIP archive. This audit-ready package ensures reproducibility, traceability, and compliance, completing Anya's validation mission.

## 2. Code Requirements

### Import Statement

The application will import all necessary functions and constants directly from `source.py`.
...
import hashlib
import json
import matplotlib.pyplot as plt # For rendering SHAP plots
import shap # For SHAP specific functions like TreeExplainer, summary_plot, waterfall_plot
from sklearn.model_selection import train_test_split # Used by source.py's internal logic for X_train_exp/X_test_exp

# Import all functions and necessary constants from source.py
# This assumes source.py contains only function definitions and constants,
# without top-level workflow execution that would run immediately on import.
from source import (
...
# This mimics the initial data generation block from source.py.
# It runs once in the global scope of the Streamlit app if the files are not found,
# ensuring `sample_credit_model.pkl` and `sample_credit_data.csv` are available
# for users to "upload" or for local testing.
...
`st.session_state` will be extensively used to preserve the application's state across user interactions and page navigations.

**Initialization:**

```python
if 'page' not in st.session_state:
    st.session_state.page = 'Welcome' # Current active page/view
if 'model' not in st.session_state:
    st.session_state.model = None # Loaded model object
if 'data' not in st.session_state:
    st.session_state.data = None # Loaded full dataset
if 'X' not in st.session_state:
    st.session_state.X = None # Features DataFrame
if 'y' not in st.session_state:
    st.session_state.y = None # Target Series
if 'model_hash' not in st.session_state:
    st.session_state.model_hash = None # SHA-256 hash of the loaded model
if 'data_hash' not in st.session_state:
    st.session_state.data_hash = None # SHA-256 hash of the loaded data
if 'explainer' not in st.session_state:
    st.session_state.explainer = None # Initialized SHAP TreeExplainer object
if 'global_importance_df' not in st.session_state:
    st.session_state.global_importance_df = pd.DataFrame() # DataFrame for global feature importance
if 'global_shap_values' not in st.session_state:
    st.session_state.global_shap_values = None # Raw SHAP values for global plots
if 'local_explanations_data' not in st.session_state:
    st.session_state.local_explanations_data = {} # Dictionary of local explanation results (JSON-serializable)
if 'local_shap_explanations_list' not in st.session_state:
    st.session_state.local_shap_explanations_list = [] # List of shap.Explanation objects for local waterfall plots
if 'counterfactual_result' not in st.session_state:
    st.session_state.counterfactual_result = {} # Dictionary of counterfactual results (JSON-serializable)
if 'explanation_dir' not in st.session_state:
    st.session_state.explanation_dir = None # Unique directory for current session's artifacts
if 'run_id' not in st.session_state:
    st.session_state.run_id = None # Unique ID for the current validation run
if 'zip_archive_path' not in st.session_state:
    st.session_state.zip_archive_path = None # Path to the generated audit ZIP file
if 'selected_instance_for_local_exp' not in st.session_state:
    st.session_state.selected_instance_for_local_exp = None # Index of instance chosen for local explanation
if 'denied_instance_for_cf_idx' not in st.session_state:
    st.session_state.denied_instance_for_cf_idx = None # Index of denied instance for counterfactuals
if 'X_train_exp' not in st.session_state:
    st.session_state.X_train_exp = None # Features subset for SHAP explainer background (training data)
if 'X_test_exp' not in st.session_state:
    st.session_state.X_test_exp = None # Features subset for SHAP plots (test data or sample)
if 'current_uploaded_model_name' not in st.session_state:
    st.session_state.current_uploaded_model_name = None # Name of the currently uploaded model file
if 'current_uploaded_data_name' not in st.session_state:
    st.session_state.current_uploaded_data_name = None # Name of the currently uploaded data file
```

**Updates and Reads Across Pages:**

*   **`st.session_state.page`**: Updated via sidebar radio buttons. Read to conditionally render the main content area.
*   **`st.session_state.model`, `st.session_state.data`, `st.session_state.X`, `st.session_state.y`**: Initialized and updated on 'Setup & Data' page after file upload and "Load and Hash Artifacts" button click. Read on all subsequent pages (Global, Local, Counterfactuals, Summary) to enable explanation generation.
*   **`st.session_state.model_hash`, `st.session_state.data_hash`**: Initialized and updated on 'Setup & Data' page. Read on 'Summary & Audit' page for generating `config_snapshot.json` and `explanation_summary.md`.
*   **`st.session_state.explainer`**: Initialized on 'Setup & Data' page once the model is loaded. Read on 'Global Explanations' and 'Local Explanations' pages.
*   **`st.session_state.global_importance_df`, `st.session_state.global_shap_values`**: Updated on 'Global Explanations' page after "Generate Global Explanations" button click. Read to display results on the same page.
*   **`st.session_state.local_explanations_data`, `st.session_state.local_shap_explanations_list`**: Updated on 'Local Explanations' page after "Generate Local Explanations" button click. Read to display results on the same page.
*   **`st.session_state.counterfactual_result`**: Updated on 'Counterfactuals' page after "Generate Counterfactuals" button click. Read to display results on the same page.
*   **`st.session_state.explanation_dir`, `st.session_state.run_id`**: Initialized on 'Setup & Data' page upon loading artifacts. `st.session_state.explanation_dir` is passed to all `source.py` functions that generate files to ensure they are saved in the correct, unique session directory. Read on 'Summary & Audit' page for bundling artifacts.
*   **`st.session_state.zip_archive_path`**: Updated on 'Summary & Audit' page after "Export All Audit-Ready Artifacts" button click. Read to provide a download button for the generated ZIP file.
*   **`st.session_state.selected_instance_for_local_exp`**: (Implicitly set by multiselect widget in Local Explanations page).
*   **`st.session_state.denied_instance_for_cf_idx`**: Populated on 'Local Explanations' page (if a denied instance is selected) or initialized on 'Counterfactuals' page to provide a default selection for counterfactual analysis.
*   **`st.session_state.X_train_exp`, `st.session_state.X_test_exp`**: Initialized on 'Setup & Data' page. Read on 'Global Explanations' and 'Local Explanations' pages for SHAP background data and plot rendering.
*   **`st.session_state.current_uploaded_model_name`, `st.session_state.current_uploaded_data_name`**: Updated on 'Setup & Data' page upon file upload. Used to detect if new files are selected, triggering a reset of relevant session state variables.

### Application Structure and Flow

The Streamlit application will simulate a multi-page experience using a sidebar radio button for navigation.

**Global Configuration:**

```python
st.set_page_config(layout="wide", page_title="Model Explainability Workbench")
...
#### 1. Welcome Page (`st.session_state.page == 'Welcome'`)

**Markdown:**

```python
st.title("Model Explanation & Explainability Control Workbench")
st.markdown(f"") # For spacing
...
#### 2. Setup & Data Page (`st.session_state.page == 'Setup & Data'`)
...
**Markdown (Explanation of Execution):**
...
#### 3. Global Explanations Page (`st.session_state.page == 'Global Explanations'`)
...
if st.session_state.model is not None and st.session_state.X_train_exp is not None and st.session_state.explainer is not None and st.session_state.explanation_dir is not None:
    ...
    if not st.session_state.global_importance_df.empty:
        ...
        if st.session_state.global_shap_values is not None and st.session_state.X_test_exp is not None:
            ...
            # Ensure sample_data for plotting is trimmed to match shap_values length
            sample_data_for_plot = st.session_state.X_test_exp.head(st.session_state.global_shap_values.shape[0])
            display_shap_plot(shap.summary_plot, st.session_state.global_shap_values, sample_data_for_plot, plot_type="bar")
    else:
...
#### 4. Local Explanations Page (`st.session_state.page == 'Local Explanations'`)
...
if st.session_state.model is not None and st.session_state.X is not None and st.session_state.y is not None and st.session_state.explainer is not None and st.session_state.explanation_dir is not None:
    ...
    if len(st.session_state.X) > 0 and st.session_state.model is not None and 1 in st.session_state.model.classes_:
...
#### 5. Counterfactuals Page (`st.session_state.page == 'Counterfactuals'`)
...
#### 6. Summary & Audit Page (`st.session_state.page == 'Summary & Audit'`)
...
**Widgets and Interaction with `source.py` (Summary):**
    ...
    if st.button("Generate Explanation Summary Report", key='generate_summary_button'):
        with st.spinner("Generating summary report..."):
            try:
                ...
                # Note: The source.py function uses global model_hash_val/data_hash_val.
                # If these were not updated by user upload, the summary will reflect the sample data hashes.
...
**Widgets and Interaction with `source.py` (Audit Trail):**
