...
def generate_global_shap_explanation(model, X_data, feature_names, explanation_dir):
    ...
    # Ensure feature_names and importance are 1D arrays
    feature_names_1d = np.array(feature_names).flatten()
    importance_1d = np.abs(shap_values_for_class_1).mean(axis=0).flatten()
    print(
        f"[DEBUG] feature_names_1d.shape: {feature_names_1d.shape}, importance_1d.shape: {importance_1d.shape}")
    # Fix: Only use the first N elements of importance_1d where N = len(feature_names_1d)
    N = len(feature_names_1d)
    if len(importance_1d) > N:
        print(
            f"[DEBUG] Truncating importance_1d from {len(importance_1d)} to {N}")
        importance_1d = importance_1d[:N]
    elif len(importance_1d) < N:
        raise ValueError(
            f"importance_1d has fewer elements ({len(importance_1d)}) than feature_names ({N})")
    feature_importance = pd.DataFrame({
        'feature': feature_names_1d,
        'importance': importance_1d
    ...
    shap.summary_plot(shap_values_for_class_1, sample_data,
...
def generate_local_shap_explanations(model, X_data, instances_to_explain, explainer, explanation_dir):
    ...
    if positive_class_val in model.classes_:
        ...
        print(
            f"Warning: Model trained on classes {model.classes_}. Class {positive_class_val} not found. Probability of approval will be 0.")
...
if len(X) > 0 and positive_class_val_main_block in credit_model.classes_:
    positive_class_idx_for_proba_main_block = np.where(
        credit_model.classes_ == positive_class_val_main_block)[0][0]
    probabilities_for_borderline = credit_model.predict_proba(
        X)[:, positive_class_idx_for_proba_main_block]
else:
    if len(X) > 0:
        print(
            f"Warning: Model classes are {credit_model.classes_}. Class {positive_class_val_main_block} not learned, so probability of approval is always 0 for borderline calculation.")
...
instances_for_local_explanation = list(selected_unique_indices)[
    :3]  # Take up to 3 unique indices
...
def generate_counterfactual_explanation(model, X_data, feature_names, instance_idx, desired_class, explanation_dir):
    ...
    d = dice_ml.Data(dataframe=full_data,
                     continuous_features=feature_names, outcome_name=TARGET_COLUMN)
    ...
    if dice_exp.cf_examples_list:
        ...
        cf_pred_prob = model.predict_proba(cf_df.drop(columns=[TARGET_COLUMN]))[
        ...
        print(cf_df.drop(columns=[TARGET_COLUMN]))
        print(
...
def generate_explanation_summary(global_imp_df, local_exp_data, cf_exp_data, explanation_dir):
    ...
    if cf_exp_data and 'original_instance' in cf_exp_data and 'counterfactual_instance' in cf_exp_data:
        original_features_str = ', '.join([f"{k}: {v:.2f}" for k, v in cf_exp_data['original_instance'].items(
        ) if k in ['credit_score', 'income', 'debt_to_income']])
        cf_features_str = ', '.join([f"{k}: {v:.2f}" for k, v in cf_exp_data['counterfactual_instance'].items(
        ) if k in ['credit_score', 'income', 'debt_to_income']])
...
generate_explanation_summary(
    global_importance_df, local_explanations_data, counterfactual_result, EXPLANATION_DIR)
...
if len(X) > 0 and positive_class_val_main_block in credit_model.classes_:
    ...
    if model_predict_proba_output.shape[1] > positive_class_idx_for_proba_main_block:
        ...
        print(
            f"Warning: credit_model.predict_proba(X) output has unexpected shape {model_predict_proba_output.shape}. Borderline calculation uses default 0.0 probability.")
else:
    if len(X) > 0:
        print(
            f"Warning: Model classes are {credit_model.classes_}. Class {positive_class_val_main_block} not learned, so probability of approval is always 0 for borderline calculation.")
...
denied_instance_for_cf_idx = next(
    (idx for idx in instances_for_local_explanation if y.loc[idx] == 0), None)
...
config_file = create_config_snapshot(
    model_hash_val, data_hash_val, RANDOM_SEED, EXPLANATION_DIR)
